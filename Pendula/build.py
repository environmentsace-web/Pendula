...
def _fetch_day(day, today, stat, lats, lons, sources) -> dict:
    ...
    grid = dict(lat=lats, lon=lons)

    def rg(da):
        return da.interp(**grid).values
    ...
    return dict(
        ...
        depth_levels=bgc["chl3d"].depth.values,
        theta=rg(phy["temp3d"]["thetao"].sel(**sel)),
        salinity=rg(phy["sal3d"]["so"].isel(depth=0).sel(**sel)),
        u=rg(phy["currents"]["uo"].isel(depth=0).sel(**sel)),
        v=rg(phy["currents"]["vo"].isel(depth=0).sel(**sel)),
...
def _synthetic_day(day, stat, lats, lons) -> dict:
    ...
    return dict(
        ...
        chl3d=chl3d, depth_levels=levels, theta=theta, salinity=sal,
